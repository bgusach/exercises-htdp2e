...
(define (compose-h loi)
  (local
    (; Image Image -> Image
     (define (merge-img img acc)
       (beside acc img)
       ))

  (foldl merge-img empty-image loi)
  ))


; [List-of Image] -> Image
; Composes horizontally from left to right the list of images
; NOTE: same as previous function, shorter implementation
(check-expect (compose-h-v2 '()) empty-image)
(check-expect (compose-h-v2 (list sq1 sq2)) (beside sq1 sq2))
(define (compose-h-v2 loi)
  ; Although we are procesing images from left to right,
  ; and adding the next image to the right of the current one,
  ; we use foldr (which processes lists from right to left)
  ; because the foldl and beside signatures are "reversed".
  ; In the previous exercise a helper function was used to 
  ; prevent this reversing.
